...
const activityIcon:{[action:string]:string} = { start:'▶', stop:'■', shutdown:'■', restart:'↻', error:'!', save:'✎', upload:'↑', download:'↓', mkdir:'+', touch:'+', rename:'→', delete:'×' };
...
export default function Dashboard() {
  ...
  const [view,setView] = useState<'overview'|'containers'|'files'|'activity'>('overview');
  ...
  useEffect(() => {
    if (!logView) return;
    const onKey=(event:KeyboardEvent)=>{ if(event.key==='Escape') setLogView(null); };
  ...
  async function showLogs(name:string,tail=logTail){
    if(!token)return;setBusy(`读取 ${name} 日志`);setBusyKey(name);
    try{
  ...
  return <main className="app-shell">
    ...
    {logView&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setLogView(null)}><section className="log-modal" role="dialog" aria-modal="true" aria-label={`${logView.name} 日志`} onMouseDown={event=>event.stopPropagation()}>
      <header><div><small>容器日志 · 最近 {logTail} 行 · Esc 关闭</small><h2>{logView.name}</h2></div>
        <div className="log-tools">
          ...
          <button onClick={()=>setLogView(null)} aria-label="关闭">×</button>
        </div></header>
